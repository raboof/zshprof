...
let update_stack from next stack =
  ...
  if (from.depth = next.depth && from.depth = List.length stack) then
    let current_frame = List.hd newstack in
    let updated_frame = { current_frame with micros_inclusive = current_frame.micros_inclusive + last_micros } in
    (updated_frame :: (List.tl newstack), [])
  else
